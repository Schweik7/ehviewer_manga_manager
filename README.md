...
6. 顶部 **选择数据库** 按钮可手动指定要操作的数据库（手机上的某个导出，或电脑
   本地 `.db` 文件）；默认仍是「最新原生导出」，无需指定
...
默认操作「最新的 EhViewer 原生导出」。如需手动指定，可加：

| 参数 | 说明 |
|------|------|
| `--db-name <文件名>` | 使用手机导出目录中的指定数据库（用 `list-db` 查看可选项） |
| `--db-file <本地路径>` | 改用电脑上的本地 `.db` 文件（优先级高于 `--db-name`） |

```bash
python main.py list-db                              # 列出手机上所有可选数据库
python main.py stats --db-name 2026-06-09-10-07-46-088.db
python main.py clean --db-file D:/backup/old.db --push
```

> 注：工具推送回手机的 `ehviewer_cleaned_*.db` 不会被默认选中，避免反复基于旧快照操作。
